    ...
    editButtons.forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            if (editFields.style.display === 'none' || editFields.style.display === '') {
                ...
                // UPDATE Method - Fetch data from backend API
                fetch(`${baseUrl}/player.html`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    // Update player data with new name and email
                    data.name = newName;
                    data.email = newEmail;

                    // Send updated data to backend
                    return fetch(`${baseUrl}/api/player/${playerId}`, {
                        method: 'PATCH',
                        headers: {
                ...
                .then(updatedData => {
                    console.log('Player updated successfully:', updatedData);

                    // Update the player's name and email in the list item
                    listItem.querySelector('.player-name').textContent = updatedData.name;
                    listItem.querySelector('.player-email').textContent = updatedData.email;
                    ...
                    if (currentId === playerId) {
                        document.getElementById('details-player-name').textContent = updatedData.name;
                        document.getElementById('details-player-email').textContent = updatedData.email;
                    }
                })
                .catch(error => {
                    console.error('There was an issue with updating the player:', error);
                });
        });
    } else {
